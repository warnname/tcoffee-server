...
my($i)=0, my(@names)=(), my $locale=0, my $distant=0, my $database, my $blast_way;
...
my($program,$database_line,$blast_line,$query_file,
   ...
   $iter,$i_tresh,$align,$matrix,$filter,$method,$orgn,$process,$quiet,$mega,$gigablast)= &OPTIONS_GET();
unless ($gigablast=~/^yes$|^no$/i) { print STDERR "invalid argument for gigaglast option : yes/no\n";exit;};
...
if    ($align==0 && $method =~/^geneid$/)  { $align=50 }
elsif ($align==0 && $method !~/^geneid$/)  {$align=1   }
else  {;}
#
if ($gigablast=~ /^yes$/i)                                 { $locale=2; $distant=0;  }
if ($method=~/^pdbid$/i && $gigablast=~ /^yes$/i)          { $database="pdb";        }
if ($method=~  /pdbid/i && $distant==1)                    {    $database="pdb";     }
if ($method=~ /geneid/i && $distant==1)                    {  unless ($database eq "nr" || $database eq "swissprot" || $database eq "pdb")  
                                                           { $database="refseq_protein";} ; }
#
if ($method=~ /profile/i && $distant==1) 
{   
    if ($out_file eq "") { $out_file ="default.profile"}    
    unless ($database eq "nr" || $database eq "swissprot" || $database eq "refseq_protein" )  { $database="pdb"; };
}

if ($method=~ /profile/i && $locale) 
{  
    if ($out_file eq '') { $out_file ="default.profile"}    
}
...
if    ($locale==1 || $locale==2) {@list_pdb= &LOCAL_BLAST ($blast_way,$database,$query_file,$Eval,$align,$method,$matrix,$filter,$process,$mega,$gigablast);   }
...
sub CONTROLE_DB_PG
{
	...
	{    
	    ...
	    while ($_=<FIC1>)  { if ($_=~ /^>.+\s/) {last;}  else {die "$database is not in fasta format \n"; } }
	    close FIC1;
	}	
    }
...
{   
    ...
    my($align)       = $opt{'hits'};                    unless ($align)       { $align=0;};  
    ...
    my($mega)        = $opt{'mega'};                            unless ($mega)         {$mega="F"};
    ...
    return ($program,$database,$blast_dir,$query_file,
	    ...
	    $iter,$i_tresh,$align,$matrix,$filter,$method,$orgn,$process,$param,$mega,$gigablast);
}
...
sub LOCAL_BLAST
{
    my ($blast_dir,$database,$query_file,$Eval,$align,$method,$matrix,$filter,$process,$mega,$gigablast)=@_;
    unless ($gigablast=~ /^yes$/i) { if ($blast_dir =~ /wu-/) { $matrix= lc($matrix); } }

    if ($method=~ /^profile$/i && $gigablast=~ /^no$/i)      
    { 
	open (COM,"$blast_dir -p blastp -d $database -i $query_file -m 6 -M $matrix -v $align -b $align -n $mega  -F $filter -e $Eval -a $process|") or die;
    }
    elsif ($method=~ /^geneid$/i && $gigablast=~ /^no$/i) 
    { 
      
	open (COM,"$blast_dir -p blastp -d $database -i $query_file  -v $align  -b $align -F $filter -n $mega -M $matrix -e $Eval -a $process|") or die;
    }

    elsif ($method=~ /^profile$|^geneid$|^pdbid$/i && $gigablast=~ /^yes$/i)
    {
	unless ($database eq "nr" || $database eq "pdb" || $database eq "refseq_protein" ) { print STDERR "\nsorry invalid database for gigablast\n";exit;} ;
	...
	open (COM,"runblast.pl1  -d $database -p blastp  -e $Eval -v $align -F F \<$query_file |");
    }
    else
    {  
	open (COM,"$blast_dir -p blastp -d $database -i $query_file -v 1 -b 1  -F $filter -e $Eval -M $matrix  -n $mega -v $align -b $align -a $process |") or die;
    }
...
{    
	...
	else { ($query,$length_query)=($intra_res[0] =~ /Query= (.+?)\s.+?\(([\d,]+) letters/smo); }
	...
	{	    
	    ...
	    {	    		
		if ($database !~ /^pdb$/i && $database !~ /swiss/i && $locale==2 || $locale==1)
		{
		...
		elsif ($database=~ /pdb/i && ($locale==1 || $locale==2))
		{	  
		    my $refseq;
...
sub PROFILE
{
    ...
    my(@list_pdb)=@$list_pdb,  my(@sortie)=();my $name2;
    my($home)=$ENV{'HOME'};
	...
	{	    
	    my($name)   =($pdb_result =~ /Query= (.+?)\(.*?letters/ismo)  or die "\nparse error in profile\n"; 
	  
	    my($name1)= ($name=~ /(.+?)\s+$/);$name1=~ s/\s//g; 
	    ($name2 = $name1)=~ s/[\|\s]/_/g;
	    ...
	    open(COM,"|t_coffee -other_pg seq_reformat -input blast_aln -in tempo_file_profile -output fasta_aln -out ${name2}.profile");	     
	    close COM;
	    push (@sortie,">$name1 _R_ ${name2}.profile\n");
