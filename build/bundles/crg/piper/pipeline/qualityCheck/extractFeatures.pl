...
my ($chr_subseq , $promoters_size , $feature2extract  ,  $experimentName , $pipelineDirName) = options();
...
if (($feature2extract eq 'exons') or ($feature2extract eq 'all')){
  if (-d "${outDir}/exons"){(system "rm -rf ${outDir}/exons") == 0 or die "Error[extractFeatures.pl]! Cannot remove ${outDir}/exons\n";}
  (system "mkdir -p ${outDir}/exons") == 0 or die "Error[extractFeatures.pl]! cannot execute\nmkdir -p ${outDir}/exons\n$!\n";
  splitySpeciesByQuery("exons");
  GTF_TO_FASTA_transcript("${outDir}/exons/","no");
}

#PROMOTERS
if (($feature2extract eq 'promoters') or ($feature2extract eq 'all')){
  if (-d "${outDir}/promoters"){(system "rm -rf ${outDir}/promoters") == 0 or die "Error[extractFeatures.pl]! Cannot remove ${outDir}/promoters\n";}
  (system "mkdir -p ${outDir}/promoters") == 0 or die "Error[extractFeatures.pl]! cannot execute\nmkdir -p ${outDir}/promoters\n$!\n";
  #TAKE GTF
  opendir (EO,"$exonerate_out") or die "Error[extractFeatures.pl]! Cannot open $exonerate_out\n$!\n";
  while (my $exGtfFile = readdir(EO)){
    next if (($exGtfFile eq '.') || ($exGtfFile eq '..') || ($exGtfFile=~/\.fa$/));
    my @sortedTags    = takeSortedTags("${exonerate_out}/${exGtfFile}");
    my %infoGtf       = readingGTFexons("${exonerate_out}/${exGtfFile}");
    my %infoPromoters = gtfExons2gtfPromoters(%infoGtf);
    printGtf(\@sortedTags,\%infoPromoters,'promoters');
  }
  closedir EO;
  #TAKE FASTA
  GTF_TO_FASTA_transcript("${outDir}/promoters/","no");
}

#EXON GTF TO INTRONS
if (($feature2extract eq 'introns') or ($feature2extract eq 'all')){
  if (-d "${outDir}/introns"){(system "rm -rf ${outDir}/introns") == 0 or die "Error[extractFeatures.pl]! Cannot remove ${outDir}/introns\n";}
  (system "mkdir -p ${outDir}/introns") == 0 or die "Error[extractFeatures.pl]! cannot execute\nmkdir -p ${outDir}/introns\n$!\n";
  #TAKE GTF
  opendir (EO,"$exonerate_out") or die "Error[extractFeatures.pl]! Cannot open $exonerate_out\n$!\n";
  while (my $exGtfFile = readdir(EO)){
    next if (($exGtfFile eq '.') || ($exGtfFile eq '..') || ($exGtfFile=~/\.fa$/));
  ...
  closedir EO;
  #TAKE FASTA
  GTF_TO_FASTA_transcript("${outDir}/introns/","yes");
}

#EXON GTF TO TRANSCRIPTS
if (($feature2extract eq 'transcripts') or ($feature2extract eq 'all')){
  if (-d "${outDir}/transcripts"){(system "rm -rf ${outDir}/transcripts") == 0 or die "Error[extractFeatures.pl]! Cannot remove ${outDir}/transcripts\n";}
  (system "mkdir -p ${outDir}/transcripts") == 0 or die "Error[extractFeatures.pl]! cannot execute\nmkdir -p ${outDir}/transcripts\n$!\n";
  #TAKE FASTA   you have to do it first for the special case of transcript. This way it is correct
  splitySpeciesByQuery("transcripts");
  GTF_TO_FASTA_transcript("${outDir}/transcripts/","yes");
  (system "rm -rf ${outDir}/transcripts/*gtf") == 0 or die "Error[extractFeatures.pl]! Cannot execute:\nrm -rf ${outDir}/transcripts/*gtf\n$!\n";
  #TAKE GTF
  opendir (EO,"$exonerate_out") or die "Error[extractFeatures.pl]! Cannot open $exonerate_out\n$!\n";
  while (my $exGtfFile = readdir(EO)){
    next if (($exGtfFile eq '.') || ($exGtfFile eq '..') || ($exGtfFile=~/\.fa$/));
...
#FUNCTIONS
sub options {
  my ($chr_subseq , $promoters_size , $feature2extract , $experimentName , $pipelineDirName);
  my $spyExperiment           = 1;
  my $spyPipelineDir          = 1;
  ...
  return ($chr_subseq , $promoters_size , $feature2extract ,  $experimentName , $pipelineDirName);
}

sub acceptedVariableSpace {
  my %space = ('-experiment' => 1 , '-pipeline_dir' => 1   , '-feature' => 1 , '-promoters_size' => 1 , '-chr_subseq' => 1);
  foreach my $field (0..$#ARGV){
...
sub help_message {
my $helpMessage = "\nNAME
extractFeatures.pl - it extracts specific features of the EXONERATE_OUT files. The features supported so far are promoters, introns and transcripts

SYNOPSIS
extractFeatures.pl -experiment -pipeline_dir [-promoters_size -chr_subseq -feature]
...
sub readingGTFexons {
  ...
  foreach my $line (<IN>){
	...
	if ($line=~/^(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(.*)$/){
	    ...
	    else {die "Error[extractFeatures.pl]! gtf file in wrong format. Impossible to find the transcript_id in line:\n$line\n when it is mandatory for this format\n";}
...
sub takeSortedTags {
  ...
  while (<E>){
    ...
    else{die "Error[extractFeatures.pl]! line\n$_\nin file $inputGtf does not contain the transcript_id field\n";}
  }
  close E;
  return @sortedTags;
}
...
sub splitySpeciesByQuery {
  ...
  while (my $exGtfFile = readdir(EO)){
    next if (($exGtfFile eq '.') || ($exGtfFile eq '..') || ($exGtfFile=~/\.fa$/));
    my $species;
    if ($exGtfFile=~/(\S+)\.ex\.gtf$/){
      $species = $1;
    }
    else {die "Error[extractFeatures.pl]! Cannot take the species name";}
    open (E, "<${exonerate_out}/${exGtfFile}") or die "Error[extractFeatures.pl]! Cannot open ${exonerate_out}/${exGtfFile}\n$!\n";
    while (<E>){
      chomp $_;
	...
	(system "echo \"$_\" >> ${outDir}/${outFeatureFolder}/${id}.ex.gtf") == 0 or die "Error[extractFeatures.pl]! Cannot execute\necho \"$_\" >> ${outDir}/${outFeatureFolder}/${id}.ex.gtf\n$!\n";
      ...
      else{die "Error[extractFeatures.pl]! line\n$_\nin file $exGtfFile does not contain the transcript_id field\n";}
    }
    close E;
  }
  closedir EO;
...
sub printGtf {
  my ($sortedTags_p , $info_p  , $type) = @_;
  my @sortedTags = @{$sortedTags_p};
  my %info       = %{$info_p};
  my ($f,$t);
  if ($type eq 'promoters')  {$f = 'promoters'  ; $t = 'pr';}
  if ($type eq 'transcripts'){$f = 'transcripts'; $t = 'tx';}
  if ($type eq 'introns')    {$f = 'introns'    ; $t = 'in';}

  if (($type eq 'promoters') or ($type eq 'transcripts')){
...
sub GTF_TO_FASTA_transcript{
  ...
  while (my $gtfFile = readdir(G)){
    ...
    foreach my $line (<GTF>){
      my ($chr , $start , $end , $strand , $geneId , $transcriptId);
      if ($line =~/^(\S+)\s+\S+\s+\S+\s+(\S+)\s+(\S+)\s+\S+\s+(\S+)\s+\S+\s+/){
	$chr          = $1;
	$start        = min ($2, $3);
	$end          = max ($3, $2);
	...
	$species = $1;
      ...
      $transcriptInfo{$transcriptId}{$species}{'chr'}    = $chr;
      $transcriptInfo{$transcriptId}{$species}{'strand'} = $strand;
      my %exon = ("start" => $start , "end" => $end);
      push (@{$transcriptInfo{$transcriptId}{$species}{'exons'}} , \%exon);
    }
    close GTF;
  }
...
sub printFunc {
  ...
  foreach my $transcriptId (keys %transcriptInfo){
    open (OUT,">${gtfDir}/${transcriptId}.fa") or die "Error[extractFeatures.pl]!cannot open ${gtfDir}/${transcriptId}.fa :$!\n";
    foreach my $species (keys %{$transcriptInfo{$transcriptId}}){
      my $fileChr = "${pipelineDirName}/allGenomeInfo/${species}/chr/" . $transcriptInfo{$transcriptId}{$species}{'chr'};
      die "Error[extractFeatures.pl]! cannot find the $fileChr for the transcript $transcriptId, maybe different format $!\n" unless (-f "$fileChr");
      my $string;
      @{$transcriptInfo{$transcriptId}{$species}{'exons'}} = sort {$a->{"start"} <=> $b->{"start"} } @{$transcriptInfo{$transcriptId}{$species}{'exons'}};

      #concatenate fasta
      if ($concatenate eq "yes"){
	foreach my $exon (@{$transcriptInfo{$transcriptId}{$species}{'exons'}}){
	...
	print OUT ">$species\n";
	if ($transcriptInfo{$transcriptId}{$species}{'strand'} eq '-'){
