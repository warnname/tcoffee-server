...
my ($stranded , $pickUp_gtf_map_randomly , $extension_on_gtf_annotations , $gtf_annotations , $gtfToRandomize , $randomizations , $species , $shuffleBed , $shuffleBed_gap ,  $experimentName , $pipelineDirName) = options();
acceptedVariableSpace();
my $assemblyDir = "${pipelineDirName}/allGenomeInfo/${species}/chr/";
my $ovName      = basename($gtf_annotations);
my $outDir      = "${pipelineDirName}/experiments/${experimentName}/qualityCheck/overlapStatistics/${ovName}/";
...
my $gtf_map              = "${pipelineDirName}/experiments/${experimentName}/EXONERATE_OUT/${species}".'.ex.gtf';
my $hypergeometric_test  = "${pipelineDirName}/qualityCheck/hypergeometric_test.pl";
...
if (defined $pickUp_gtf_map_randomly){
  open (GTF_MAP,"<$gtf_map") or die "Error[overlap_statistics.pl]! Cannot read $gtf_map \n$!\n";
  @allGtfMap = <GTF_MAP>;
  close GTF_MAP;
}
...
sub help_message {
my $helpMessage = "\nNAME
overlap_statistics.pl - Given an annotations gff(or gtf) file (we call gtf_annotations), this script measure the overlap between the .gtf EXONERATE_OUT gtf of a certain species (we call gtf_map)  and the given gtf_annotation (cuflink transcripts from RNAseq, chromatine markups annotations). The overlap is measured in terms of #overlapping transcripts,  #overlapping exons and #ovarlapping nucleotides. Then two kinds of statistical significance test are run:
...
sub options {
  my ($stranded , $pickUp_gtf_map_randomly , $extension_on_gtf_annotations , $gtf_annotations , $gtfToRandomize , $randomizations , $species , $shuffleBed , $shuffleBed_gap ,  $experimentName , $pipelineDirName);
  my $spyStranded                     = 1;
  ...
  return ($stranded , $pickUp_gtf_map_randomly , $extension_on_gtf_annotations , $gtf_annotations , $gtfToRandomize , $randomizations , $species , $shuffleBed , $shuffleBed_gap ,  $experimentName , $pipelineDirName);
}
sub acceptedVariableSpace {
  my %space = ('-stranded' => 1 , '-pickUp_gtf_map_randomly' => 1 , '-extension_on_gtf_annotations' => 1 , '-gtf_annotations' => 1 ,'-gtfToRandomize' => 1 , '-randomizations' => 1 , '-experiment' => 1 , '-pipeline_dir' => 1 , '-species' => 1 , '-shuffleBed' => 1,  '-shuffleBed_gap' => 1 );
  foreach my $field (0..$#ARGV){
