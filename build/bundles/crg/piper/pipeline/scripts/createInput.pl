  ...
  foreach my $line (<IN>){
	...
	if ($line=~/^(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(.*)$/){
	    ...
	    if (($unstrandedGTFname eq 'on') && ($strand eq '.')){
	      $group =~s/transcript_id \"[^\"]+\"/transcript_id \"${transcript_id}_nonStandPlus\"/;
	      $group =~s/gene_id \"[^\"]+\"/gene_id \"${gene_id}_nonStandPlus\"/;
	      my  %hash1 = (
			   "chr"           => $chr,
			   "source"        => $source,
			   "feature"       => $feature,
	      ...
	      push (@{$infoGtf{"${transcript_id}_nonStandPlus"}}, \%hash1);
	      $group =~s/transcript_id \"[^\"]+\"/transcript_id \"${transcript_id}_nonStandMinus\"/;
	      $group =~s/gene_id \"[^\"]+\"/gene_id \"${gene_id}_nonStandMinus\"/;
	      my  %hash2 = (
			   "chr"           => $chr,
			   "source"        => $source,
			   "feature"       => $feature,
	      ...
	      push (@{$infoGtf{"${transcript_id}_nonStandMinus"}}, \%hash2);
...
#   my ($transcript) = @_;
#   $infoGtf{"${transcript}_nonStandPlus"}  = $infoGtf{$transcript};
#   foreach my $exon (0..$#{$infoGtf{"$transcript"}}){
#     foreach my $field (keys %{$infoGtf{"$transcript"}[$exon]}){
#       my $value = $infoGtf{"$transcript"}[$exon]{$field};
#       $infoGtf{"${transcript}_nonStandMinus"}[$exon]{$field} = $value;
#     }
#   }
#   delete $infoGtf{$transcript};

#   foreach my $exon (0..$#{$infoGtf{"${transcript}_nonStandPlus"}}){
#     $infoGtf{"${transcript}_nonStandPlus"}[$exon]->{'strand'} = '+';
#   }
#   foreach my $exon (0..$#{$infoGtf{"${transcript}_nonStandMinus"}}){
#     $infoGtf{"${transcript}_nonStandMinus"}[$exon]->{'strand'} = '-';
#   }
# }
