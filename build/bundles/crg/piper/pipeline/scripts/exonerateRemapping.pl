...
my $extensionFile      = "${pipelineDirName}/experiments/${experimentName}/CONFIG/exonerateExtensionFile";
my $clusterConfigName  = "${pipelineDirName}/experiments/${experimentName}/CONFIG/clusterConfig";
my $exonerateOutDir    = "${pipelineDirName}/experiments/${experimentName}/EXONERATE_OUT/";
...
sub exonerate {
  ...
  if ($exoOutput=~/vulgar:\s*(\S+)\s+(\d+)\s+(\d+)\s+([+-])\s+(\S+)\s+(\d+)\s+(\d+)\s+([+-])\s+(\S+)\s+(.*)$/){
    ...
    my $switch = checkLength(length ($allTranscripts{$queryName}) , length ($nucleotides));
...
sub checkLength{
  my ($q_length , $t_length) = @_;
  my $minLength = sprintf("%.0f", ($q_length / 100) * $exonerateMinPercentLength); 
  my $switch = 0;
  $switch++ if ($t_length < $minLength);                                                            #print "ACTUAL $t_length  MIN LENGTH!! $minLength\n"; ####DEBUG
  return $switch;
}
