...
def timeout = Time.parseDuration( context['settings.piper.max.duration'] ?: '3d' ) *1000
def cmdline = context['settings.piper.cmdline'] ?: ''
// append the 'qsub' job-id
if( cmdline?.toString().trim().contains('wrap.sh') ) cmdline += " -N 'p-${context['_rid']}' "
...
genomesFile.eachLine { String line ->
   ...
   if( cacheFolder.exists() ) {
       // when the a cachef formatted genomes exists use it, 
	   // create a link to the current scratch folder    
      "ln -s ${cacheFolder} ${alias}".execute(null, allGenomeInfoFolder)
   }
   else {
	 // keep in this map all formatted genomoes that are candidate to be moved 
	 // to the cached
	 moveToCache[alias] = new File(allGenomeInfoFolder, alias) 
   }
...
startPipeline.pl -genomes ${genomesFile} -query ${query} -experiment exp_1 -blast blastn
...
if( logFile.exists() ) {
	...
	Logger.warn "Missing script execution file: '$logFile'"
}
...
/* 
 * Before exit move the local formatted genomes to the cached  
 * Note: for each genome moved to the cache folder, a symlink is created back
 */

Logger.debug("Piper finalization: move the followings genomes to cache: ${moveToCache}")

moveToCache.each { String alias, File file -> 
	
   File cachedGenome = new File(formattedGenomesFolder,alias)
   
   if( !cachedGenome.exists() ) {
	   "mv ${file.absolutePath} ${cachedGenome}" .execute(null, allGenomeInfoFolder)
	   "ln -s ${cachedGenome} ${alias}" .execute(null, allGenomeInfoFolder)
   }
