...
paste ./coefficients.pro.txt ./data/bait.test | awk '{for(i=2;i<=NF;i++){printf "%f\t", $i*$1} printf "\n"}' | awk '{for(i=1;i<=NF;i++){a[i]=a[i]+$i}} END{for(i=1;i<=NF;i++){printf "%f\t", a[i]} printf "\n"}' > ./data/bait.combined.txt
paste ./coefficients.rna.txt ./data/prey.test | awk '{for(i=2;i<=NF;i++){printf "%f\t", $i*$1} printf "\n"}' | awk '{for(i=1;i<=NF;i++){a[i]=a[i]+$i}} END{for(i=1;i<=NF;i++){printf "%f\t", a[i]} printf "\n"}' > ./data/prey.combined.txt
