...
my ($motevo , $motevo_tree , $motevo_wm_file , $motevo_runUFE , $motevo_reference , $t_coffee  ,  $experimentName , $pipelineDirName) = options();
my $outDir        = "${pipelineDirName}/experiments/${experimentName}/qualityCheck/alignPromoters/";
my $promotersDir  = "${pipelineDirName}/experiments/${experimentName}/qualityCheck/extractedFeatues/promoters/";
...
#FUNCTIONS
sub options {
  my ($motevo , $motevo_tree_file , $motevo_wm_file , $motevo_runUFE , $motevo_reference , $t_coffee , $experimentName , $pipelineDirName);
  my $spyExperiment           = 1;
  my $spyPipelineDir          = 1;
  my $spyT_coffee             = 1;
  my $spyMotevo               = 1;
  ...
  return ($motevo , $motevo_tree_file , $motevo_wm_file , $motevo_runUFE , $motevo_reference , $t_coffee  ,  $experimentName , $pipelineDirName);
}

sub acceptedVariableSpace {
  my %space = ('-motevo' => 1 , '-motevo_tree_file' =>1 , '-motevo_wm_file' => 1 , '-experiment' => 1 , '-pipeline_dir' => 1   , '-t_coffee' => 1 , '-motevo_runUFE' => 1 , '-motevo_reference' => 1);
  foreach my $field (0..$#ARGV){
...
DESCRIPTION
   * WARNING1: if you run the Motevo prediction bear in ming that the UFE must be estimated on a tree where the maximum number of species recommended is 10 (otherwise it takes too much)
   * The species of the MSA that will be used for motif prediction are the ones specified by the Newick tree.
   * MSA containing other species will be temporary re-written so that just the pecies specified in the motevo-tree are used

   * WARNING2: the specified species tree must hace species name that can be found in the MSA headers
