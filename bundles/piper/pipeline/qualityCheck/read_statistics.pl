...
#GLOBAL VARIABLES
my ($splitMode , $randomizations , $shuffleBed , $shuffleBed_gap , $millionMappedReads , $tmp_split_name , $exonGtfSplits , $cluster , $species , $stranded , $bamtools , $bamdir ,   $experimentName , $pipelineDirName) = options();
acceptedVariableSpace();
$randomizations++ if ($randomizations > 0);
my $bamdir_name = basename($bamdir);
my $outDir      = "${pipelineDirName}/experiments/${experimentName}/qualityCheck/reads_statistics/${bamdir_name}/";
my ($tx_gtf , $gtf_map , %tx_info , %ex_info , @allExons , @allSplitNames , $clusterConfigLine ,@clusterHeaderLines );
$gtf_map = "${pipelineDirName}/experiments/${experimentName}/EXONERATE_OUT/${species}.ex.gtf";
my $count                = 0;
(system "rm -rf $outDir") == 0 or die "Error[reads_statistics.pl]! cannot create theremove directory $outDir  \n$!\n" if ((-d $outDir)&&($splitMode eq 'off')) ;
...
#TAKE ASSEMBLY INFO
my ($shuffleBed_chrSize , %gtf_map_info_ex , %gtf_map_info_tx , $gtf_map_block , $numberTranscripts , @deltas4block , @blocks , @blockStarts);
if ($randomizations > 0) {
...
SYNOPSIS
perl read_statistics.pl -species -bamdir -experiment -pipeline_dir [-randomizations -shuffleBed -shuffleBed_gap -tmp_split_name -exonGtfSplits -cluster -bamtools -stranded  -millionMappedReads -help]
...
1- considering the EXONERATE transcripts, any time the exon of one transcript overlap with the exon of another, the two transcript will be joined in the same \"block\". So each block is a collection of overlapping transcripts (that overlap at exonic level).
...
OPTIONS
  ...
  *-experiment = <name>
  experiment name (the species.ex.gtf will be taken by exprerimentName/EXONERATE_OUT)

  *-pipeline_dir = <dir>
  pipeline position

  *-tmp_split_name = <file>
  OPTION: consider this exon gtf file insted the one found at exprerimentName/EXONERATE_OUT
...
sub options {
  my ($splitMode , $randomizations , $shuffleBed , $shuffleBed_gap ,$millionMappedReads , $tmp_split_name , $exonGtfSplits , $cluster , $species , $stranded , $bamtools , $bamdir ,   $experimentName , $pipelineDirName);
  my $spyStranded                     = 1;
  my $spySpecies                      = 1;
  my $spyBamdir                       = 1;
  my $spyBamtools                     = 1;
  ...
  die "Error[overlap_statistics.pl]! Please provide the -bamdir directory\n"                   if (! defined $bamdir);
  ...
  return ($splitMode , $randomizations , $shuffleBed , $shuffleBed_gap , $millionMappedReads , $tmp_split_name , $exonGtfSplits , $cluster , $species , $stranded , $bamtools , $bamdir ,   $experimentName , $pipelineDirName);
}

sub acceptedVariableSpace {
  my %space = ('-splitMode' => 1 , '-randomizations' => 1 , '-shuffleBed' => 1,  '-shuffleBed_gap' => 1 , '-millionMappedReads'=> 1 , '-stranded' => 1 , '-tmp_split_name' => 1 , '-exonGtfSplits' => 1 , '-cluster' => 1 , '-experiment' => 1 , '-pipeline_dir' => 1 , '-species' => 1 , '-bamtools' => 1 , '-bamdir' => 1);
  foreach my $field (0..$#ARGV){
